...
### Vercel 토큰 만료 공백과 수집 내결함성 (2026-08-31)

**사고**: 프로덕션 404 제보로 조사한 결과 실체는 8/29 밤부터의 배포 공백 — Vercel OAuth 세션 토큰(`auth.json`)의 `expiresAt`이 2026-08-29 19:42에 만료됐고, 파이프라인이 토큰을 추출해 `--token`으로 직접 주입하는 경로는 만료 시 자동 갱신이 일어나지 않아 이후 모든 배포가 "Not authorized"로 실패(2.5일간 프로덕션은 8/29 18:17 빌드·데이터로 동결). 404 자체는 배포 전환기 사이 엣지의 일시 현상이었고 조사 시점엔 정상 응답.
